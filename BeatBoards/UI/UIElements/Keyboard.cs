...
namespace BeatBoards.UI.UIElements
{
...
/23 (!!) (@@) [SPACE]/40 (##) (__)";
        ...
        public KEY this[string index]
        {
            get
            {
                ...
                //Plugin.log.Warn($"Keyboard: Unable to set property of Key  [{index}]");
        ...
        public void SetValue(string keylabel, string value)
        {
            ...
            //if (!found) Plugin.log.Warn($"Keyboard: Unable to set property of Key  [{keylabel}]");
        ...
        public void SetAction(string keyname, Action<KEY> action)
        {
           ...
           // if (!found) Plugin.log.Warn($"Keyboard: Unable to set action of Key  [{keyname}]");
        ...
        public KEYBOARD AddKeys(string Keyboard, float scale = 0.5f)
        {
            ...
            try
            {
                ...
                while (p < Keyboard.Length)
                {
                    ...
                    switch (Keyboard[p])
                    {
                        ...
                        default:
                            //Plugin.log.Warn($"Unable to parse keyboard at position {p} char [{Keyboard[p]}]: [{Keyboard}]");
                            return this;
                    }
            ...
            {
               // Plugin.log.Error($"Unable to parse keyboard at position {p} : [{Keyboard}]");
